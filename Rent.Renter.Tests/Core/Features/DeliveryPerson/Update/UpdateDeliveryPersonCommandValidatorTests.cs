...
public class UpdateDeliveryPersonCommandValidatorTests
{
    private readonly UpdateDeliveryPersonCommandValidator _validator = new();
    private readonly UpdateDeliveryPersonCommand _createDeliveryPersonCommandSample = new(Guid.NewGuid(), "Hércules", "16599855000110", "42997067713", "AB", DateTime.Parse("1997-03-22"), Consts.Base64BmpCnh);
    
    [Fact]
    public void Validate_ShouldBeInvalid_WhenIdIsEmpty()
    {
        var sample = _createDeliveryPersonCommandSample with { Id = Guid.Empty };
        var expectedErrorCount = 1;
        var expectedErrorMessage = "Id is required";
